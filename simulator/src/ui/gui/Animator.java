...
class Animator {
  ...
  private HashSet <Integer>                insRenderedToRow     = new HashSet <Integer> ();
  private HashMap <View, Integer>          prevInsRow           = new HashMap <View, Integer> ();
  private HashMap <View, ControlFlowScene> prevControlFlowScene = new HashMap <View, ControlFlowScene> ();
  ...
  /**
   * Animate drawing a line that connects two vertically aligned points that iscomprised of 3 sections: 
  ...
  private class StaggeredPath extends Sequence {
    StaggeredPath (Point from, Point to, int width, int stagger, boolean isArrow, Color color, int steps, int clearSteps) {
      ...
      add (new Line (from, c0, width, color, Math.max (1,steps*stagger/length), clearSteps));
      add (new Arc  (a0, to.y<from.y? 270: 90, to.y<from.y? -90: 90, color, Math.max (1,steps*width/length), clearSteps));
      add (new Line (c1, c2, width, color, Math.max (1,steps*dist/length), clearSteps));
      add (new Arc (a1, 180, to.y<from.y? -90: 90, color, Math.max (1,steps*width/length), clearSteps));
      if (isArrow) {
        Point c4 = new Point (to.x-ARROW_WIDTH, to.y);
        add (new Line (c3, c4, width, color, Math.max (1,steps*(stagger-ARROW_WIDTH)/length), clearSteps));
        int h0 = width/2;
        ...
        add (new Arrow (p0, p1, color, Math.max (1, steps*ARROW_WIDTH/length), clearSteps));
      } else
        add (new Line (c3, to, width, color, Math.max (1,steps*stagger/length), clearSteps));
    }
  }
  ...
  private class DrawingClip extends AShape {
    ...
    DrawingClip (ShapeGradient aShape, ColorGradient aColor, ColorGradient aClearColor) {
      super (new AComponent(-1), aShape, aColor);
  ...
  private class ControlFlowScene extends Sequence {
    Point from;
    Clip  clearClip;
    ControlFlowScene (View view, Point aFrom, Point to) {
      from=aFrom;
      ControlFlowScene prev = prevControlFlowScene.get (view);
      if (prev!=null) {
        from = prev.from;
        add               (prev.clearClip);
        clearScene.remove (prev.clearClip);
      } else {
        from.x += CONTROL_FLOW_FROM_OFFSET; 
        from.y += 7-CONTROL_FLOW_LINE_WIDTH/2; 
      }
      prevControlFlowScene.put (view, this);
      to.x += CONTROL_FLOW_TO_OFFSET;
      to.y += 7-CONTROL_FLOW_LINE_WIDTH/2;
      StaggeredPath path = new StaggeredPath (from, to,  
      ...
      add (path);
      clearClip = path.getClearClip();
      clearScene.add (clearClip);
    }
  }
  
  /**
   * Perform render public method using SceneWorker.
   */
  
  private class RenderWorker extends SceneWorker {
    private final Target  target;
    private final Type    type;
    private final View    view;
    private final int     labelCol, row, col, count;
    
    RenderWorker (Target aTarget, Type aType, View aView, int aLabelCol, int aRow, int aCol, int aCount) {
      ...
      if (visibleRect.contains (view.getCellRect (row, col, false))) {
        ...
        switch (target) {
          ...
          case INSTRUCTIONS:
            ...
            if (col>0 && !insRenderedToRow.contains (row)) {
              insRenderedToRow.add (row);
              Overlay ol = new Overlay();
              for (Map.Entry <View, Integer> vr : prevInsRow.entrySet()) {
                View    v     = vr.getKey();
                Integer prev  = vr.getValue();
                visibleRect   = v.getVisibleRect();
                Rectangle toR = v.getCellRect (row, col, false);
                if (visibleRect.contains (toR)) {
                  Rectangle fromR = v.getCellRect (prev, col, false);
                  if (!visibleRect.contains (fromR)) {
                    fromR.x -= CONTROL_FLOW_LINE_STAGGER;
                    fromR.y = Math.min (Math.max (fromR.y, visibleRect.y), visibleRect.y+visibleRect.height);
                  }
                  Point from = convertPoint (v, new Point (fromR.x, fromR.y));
                  Point to   = convertPoint (v, new Point (toR.x,   toR.y));
                  ol.add (new ControlFlowScene (v, from, to));
                }
              }
              setClip (ol);
            }
  ...
  private class ClearWorker extends SceneWorker {
    ...
    @Override public void render () {
      ...
      prevControlFlowScene.clear();
      rowRenderedView.clear();
      insRenderedToRow.clear();
    }
 ...
 void render (Target target, Type type, View view, int labelCol, int row, int col, int count) {
    if (isEnabled && !isStopped && duration>0 && activeScene==null) {
      synchronized (this) {
        try {
          activeScene = new RenderWorker (target, type, view, labelCol, row, col, count);
          activeScene.render ();
        } finally {
          activeScene=null;
        }
